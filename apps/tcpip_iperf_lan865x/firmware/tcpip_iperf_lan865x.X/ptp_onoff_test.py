...
RE_PTP_STATUS   = re.compile(r"PTP mode\s*:\s*(\S+)")
...
class PTPOnOffTestAgent:
    ...
    def _reopen_fol_port(self):
        """Close and reopen the FOL serial port.

        pyserial on Windows leaves Windows OVERLAPPED I/O handles in an
        undefined state after a prolonged read loop (_monitor_blackout).
        Reopening the port gives a guaranteed clean COM state before the
        next background thread accesses self.fol_ser.
        """
        if self.fol_ser and self.fol_ser.is_open:
            try:
                self.fol_ser.close()
            except Exception:
                pass
        ...
        try:
            self.fol_ser = open_port(self.fol_port_name)
            self.log.info(f"  FOL port reopened: {self.fol_port_name}")
        except serial.SerialException as exc:
            self.log.info(f"  ERROR: Could not reopen FOL port: {exc}")
            sys.exit(1)
    ...
    def test_step_3_start_ptp(self) -> bool:
        ...
        # Follower first — MUST complete before starting the convergence
        # thread, because both send_command and the thread call fol_ser.read()
        # and concurrent access to a pyserial port on Windows corrupts the
        # overlapped I/O state → access violation.
        ...
        # Now start the convergence monitor — from here on the background
        # thread is the sole reader of fol_ser.
    ...
    def _read_one_offset(self) -> Optional[int]:
        ...
        """
        self.fol_ser.reset_input_buffer()
        self.fol_ser.write(b"ptp_status\r\n")
        ...
        while time.monotonic() < deadline:
            ...
            if chunk:
                ...
                for m in RE_PTP_OFFSET.finditer(decoded):
                    ns = int(m.group(1) if m.group(1) is not None else m.group(2))
    ...
    def run_cycle(self, cycle_num: int) -> bool:
        ...
        # Reopen FOL port to ensure clean Windows overlapped-IO state
        # after the prolonged read loop in _monitor_blackout.
        self._reopen_fol_port()
